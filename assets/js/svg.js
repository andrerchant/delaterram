...
/*------------------------Cargar Varios Svgs------------------------*/
	svgs=$("svg");
	$.each(svgs,function(a,inside){
	    ID=$(inside).attr("id");
	    // var Snap_svg = Snap(inside[0]);

	    pathS = $("#"+ID+" path");
	    for (var i = 0; i < pathS.length; i++) {
	          ID_interno=$(pathS[i]).attr("id");
	          $("#"+ID_interno).attr({"d":vectores[ID].fijo[ID_interno]});
	    }
	});
});

/*PROTOTIPO SVGs Cambio-------------------------*/               

function SVGTransform( el, opciones ) {
  this.el = el;
  this.init();
}

SVGTransform.prototype.init = function() {
  	this.trigger = this.el;//.querySelector("button.b_menu");
  	this.shapeEl = this.el.querySelectorAll("svg");
	this.Paths=this.el.querySelectorAll("svg g#anim path");
  	this.initEvents();
	clase=$(this.shapeEl).attr("class");
	tiempo=$(this.shapeEl).attr("tiempo");
	Tanimacion=$(this.shapeEl).attr("Tanimacion");
};
...
SVGTransform.prototype.over = function() {
	...
	$.each(this.shapeEl,function(a,b){
		id_nm=$(b).attr("id");
		s=Snap(b);
		for (var i = 0; i < paths.length; i++) {
			id_linea = $(paths[i]).attr("id");
			// tiempo = $(paths[i]).attr("tiempo");
			// tipoTiempo = $(paths[i]).attr("tipoTiempo");
			// Tanimacion= $(paths[i]).attr("Tanimacion");
			linea_snap=  s.select("#"+id_linea);


		linea_snap.animate({"path":vectores[id_nm].s2[id_linea]}, 250, mina.linear)
...
//Cargar varios SVGs [Poner directo en el documento]
// En este caso donde será el objeto svg es dentro de un "figure"
//[].slice.call( document.querySelectorAll( 'figure' ) ).forEach( function( el ) { new SVGTransform(el); } );


//Cargar uno
// new SVGTransform( document.getElementById( 'menu' ));
